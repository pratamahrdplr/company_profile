...
<form action="del_kontak.php"method="post">
   ...
   // iki tambahan'e bro!
    ...
    switch ($id_admin) {
        case '2':
            $query = "SELECT * FROM tb_kontak_all WHERE status_kontak = 'belum dihubungi' AND kota_kontak IN ('surabaya', 'mojokerto', 'madiun', 'kediri', 'tulungagung', 'tuban', 'trenggalek', 'sumenep', 'sidoarjo', 'sampang', 'ponorogo', 'pamekasan', 'pacitan', 'ngawi', 'nganjuk', 'magetan', 'lamongan', 'jombang', 'gresik', 'bojonegoro', 'blitar', 'bangkalan', 'lumajang')";
            break;
        case '3':
            $query = "SELECT * FROM tb_kontak_all WHERE status_kontak = 'belum dihubungi' AND kota_kontak IN ('probolinggo', 'pasuruan', 'batu', 'malang', 'lumajang', 'jember', 'bondowoso', 'situbondo', 'banyuwangi')";
            break;
        case '4':
            $query = "SELECT * FROM tb_kontak_all WHERE status_kontak = 'belum dihubungi' AND kota_kontak IN ('tegal', 'surakarta', 'solo', 'semarang', 'salatiga', 'ambarawa', 'pekalongan', 'magelang', 'wonosobo', 'wonogiri', 'temanggung', 'slawi', 'sukoharjo', 'sragen', 'ungaran', 'purworejo', 'purbalingga', 'pemalang', 'klaten', 'kendal', 'kebumen', 'karanganyar', 'purwodadi', 'grobogan', 'demak', 'cilacap', 'brebes', 'boyolali', 'batang', 'banyumas', 'banjarnegara', 'sleman', 'yogyakarta', 'bantul', 'kulonprogo', 'purwokerto', 'juwana', 'juwono', 'jepara', 'kudus', 'pati', 'rembang', 'blora')";
            break;
        case '5':
            $query = "SELECT * FROM tb_kontak_all WHERE status_kontak = 'belum dihubungi' AND kota_kontak IN ('tegal', 'surakarta', 'solo', 'semarang', 'salatiga', 'ambarawa', 'pekalongan', 'magelang', 'wonosobo', 'wonogiri', 'temanggung', 'slawi', 'sukoharjo', 'sragen', 'ungaran', 'purworejo', 'purbalingga', 'pemalang', 'klaten', 'kendal', 'kebumen', 'karanganyar', 'purwodadi', 'grobogan', 'demak', 'cilacap', 'brebes', 'boyolali', 'batang', 'banyumas', 'banjarnegara', 'sleman', 'yogyakarta', 'bantul', 'kulonprogo', 'purwokerto', 'juwana', 'juwono', 'jepara', 'kudus', 'pati', 'rembang', 'blora')";
            break;
        case '6':
            $query = "SELECT * FROM tb_kontak_all WHERE status_kontak = 'belum dihubungi' AND kota_kontak IN ('cilacap', 'purwokerto', 'Purbalingga', 'Banjarnegara', 'Brebes', 'slawi', 'tegal', 'tasikmalaya', 'cirebon', 'banjar', 'majalengka', 'kuningan', 'indramayu', 'garut', 'ciamis')";
            break;
        default:
            $query = "select * from tb_kontak_all where status_kontak = 'belum dihubungi' ";
            break;
    }
    // iki akhire
