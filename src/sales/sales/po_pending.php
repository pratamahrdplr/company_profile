          ...
          <table id="datatable-responsive" class="table table-striped jambo_table bulk_action" cellspacing="0" width="100%">
            ...
            <tbody>
              ...
              if (($tanggal_input_awal == '') and ($tanggal_input_sampai == '')) {
                ...
                $penawaran_query = mysql_query("select * from tb_po  where id_user = '$session_user' AND status_po='pending' ") or die(mysql_error());
              } else {
                $penawaran_query = mysql_query("select * from tb_po where id_user = '$session_user'  AND status_po='pending' AND DATE_FORMAT(tanggal_po,'%m/%d/%Y')  BETWEEN '$tanggal_input_awal' AND '$tanggal_input_sampai'") or die(mysql_error());
              }
